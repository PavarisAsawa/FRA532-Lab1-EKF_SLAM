...
# Ref https://github.com/AtsushiSakai/PythonRobotics/blob/master/utils/plot.py
...
def plot_covariance_ellipse(x, y, cov, chi2=3.0, color="-r", ax=None):
    ...
    return plot_ellipse(x, y, a, b, angle, color=color, ax=ax)
...
def plot_ellipse(x, y, a, b, angle, color="-r", ax=None, **kwargs):
    ...
    **kwargs: Additional keyword arguments to pass to plt.plot or ax.plot.
    ...
    if ax is None:
        return plt.plot(px, py, color, **kwargs)
    else:
        return ax.plot(px, py, color, **kwargs)
