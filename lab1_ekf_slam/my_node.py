...
class OdometryNode(Node):
    def __init__(self):
        ...
        self.path_pub = self.create_publisher(Path, '/robot_path', 10) # Path Publisher
    ...
    def timer_callback(self):
        ...
        if self.show_animation:
            ...
            if self.ellipse is not None:
                for elem in self.ellipse:  # If it's a list of plot elements
                    elem.remove()  # Properly remove each element
            # self.ellipse = plot_covariance_ellipse(self.x, self.y, self.ekf.PEst, ax=self.ax) # Plot the covariance ellipse
            # print(self.ekf.PEst)
